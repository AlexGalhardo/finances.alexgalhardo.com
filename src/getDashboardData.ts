...
if (localStorage.getItem("galhardo_finances")) {
	...
	for (let i = 0; i < Account.transactions.length; i++) {
		if (Account.transactions[i].type === "DEPOSIT") {
			...
			if (Account.transactions[i].category === "FOOD")
				Account.total_food += Account.transactions[i].amount;
			if (Account.transactions[i].category === "SUBSCRIPTIONS")
				Account.total_subscriptions += Account.transactions[i].amount;
			if (Account.transactions[i].category === "SHOP")
				Account.total_shop += Account.transactions[i].amount;
			...
			if (Account.transactions[i].category === "HOUSE")
				Account.total_house += Account.transactions[i].amount;
...
export const investments_fixed_income = transformToBRL(
	Account.investments_fixed_income,
);
export const percentage_fixed_income = transformToFixedTwo(
	Account.investments_total
		? (Account.investments_fixed_income / Account.investments_total) * 100
		: 0,
);

export const investments_variable_income = transformToBRL(
	Account.investments_variable_income,
);
export const percentage_variable_income = transformToFixedTwo(
	Account.investments_total
		? (Account.investments_variable_income / Account.investments_total) *
				100
		: 0,
);

export const investments_criptocurrencies = transformToBRL(
	Account.investments_criptocurrencies,
);
export const percentage_criptocurrencies = transformToFixedTwo(
	Account.investments_total
		? (Account.investments_criptocurrencies / Account.investments_total) *
				100
		: 0,
);
...
export const percentage_others = transformToFixedTwo(
	Account.investments_total
		? (Account.investments_others / Account.investments_total) * 100
		: 0,
);
...
export const percentage_food = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_food / Account.total_expenses) * 100
		: 0,
);
...
export const percentage_subscriptions = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_subscriptions / Account.total_expenses) * 100
		: 0,
);
...
export const percentage_shop = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_shop / Account.total_expenses) * 100
		: 0,
);
...
export const percentage_entertainment = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_entertainment / Account.total_expenses) * 100
		: 0,
);
...
export const percentage_transport = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_transport / Account.total_expenses) * 100
		: 0,
);
...
export const percentage_house = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_house / Account.total_expenses) * 100
		: 0,
);
...
export const percentage_services = transformToFixedTwo(
	Account.total_expenses
		? (Account.total_services / Account.total_expenses) * 100
		: 0,
);
