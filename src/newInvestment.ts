...
export function newInvestment(
	...
	element.addEventListener("click", () => {
		if (
			...
			const amountInvested = transformStringInputValueMaskToNumber(
				amountInvestment.value,
			);
			...
			if (amountInvested > 0) {
				if (Account) {
					if (amountInvested <= Account.current_balance) {
						...
						if (investmentCategory.value === "OTHERS")
							Account.investments_others += amountInvested;
						...
						alert(
							"You dont have sufficient balance to make this investment!",
						);
					}
				} else {
					Account = {
