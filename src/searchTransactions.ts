		...
		if (localStorage.getItem("galhardo_finances")) {
			if (searchStartDate.value && searchFinalDate.value) {
				...
				for (let i = 0; i < account.transactions.length; i++) {
					let created_at = account.transactions[i].created_at.slice(
						0,
						10,
					);
					let dateFormated = `${created_at[3]}${created_at[4]}/${created_at[0]}${created_at[1]}/${created_at[6]}${created_at[7]}${created_at[8]}${created_at[9]}`;
					let dateFormatedTimestamp = new Date(
						dateFormated,
					).getTime();

					if (
						searchCategory.value === "ALL" &&
						dateFormatedTimestamp >=
							new Date(searchStartDate.value).getTime() &&
						dateFormatedTimestamp <=
							new Date(searchFinalDate.value).getTime()
							? true
							: account.transactions[i].category ===
						...
						transactions += `
							<li class="list-group-item list-group-item-action d-flex justify-content-between">
								<div class="me-auto">
									<h5 class="fw-bold ${colorType}">${getTransactionCategoryIcon(
										account.transactions[i].category,
									)}   ${
								...
								<div class="ms-auto">
									<h5 class="fw-bold ${colorType}">${simbolType} R$ ${transformToBRL(
				...
				document.querySelector("#ul_transactions")!.innerHTML =
					transactions;
				deleteTransaction(
					document.querySelectorAll(".button_delete_transaction")!,
				);
			} else {
				alert(
					"Please select a start date and final date to filter transactions",
				);
			}
		} else {
			document.querySelector("#ul_transactions")!.innerHTML =
				"There's no transactions available";
		}
	});
}
